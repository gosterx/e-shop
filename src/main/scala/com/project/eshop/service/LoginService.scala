...
trait LoginService [F[_]]{
  def login(authInfo: UserAuthInfo): F[String]
}
...
object LoginService {
  def of[F[_]: Sync](userRepository: UserRepository[ConnectionIO], tokenRepository: TokenRepository[ConnectionIO], transactor: Transactor[F]): LoginService[F] =
    new LoginService[F] {
      override def login(authInfo: UserAuthInfo): F[String] =
        (for {
          ...
          _ <- if (Base64.getEncoder.encodeToString(authInfo.password.getBytes) == user.password) Sync[ConnectionIO].pure()

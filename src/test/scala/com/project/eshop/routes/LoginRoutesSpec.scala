...
class LoginRoutesSpec extends AnyWordSpec with Matchers with DbSetup {
  ...
  "Request to register API" should {
    ...
    "login user" in {
      val request =
        Request[IO](method = Method.POST, uri = Uri.unsafeFromString("http://localhost:8080/api/v1/login"))
          .withEntity(
            UserAuthInfo("example", "password")
          )
      ...
      response.cookies.exists(_.name == "authToken") shouldBe true
      response.as[User].unsafeRunSync().copy(id = "1") shouldEqual User(
        "1",
        "example",
        "example@gmail.com",
